...
export default function Login() {
  ...
  return (
    <div style={{ padding: '100px 20px', maxWidth: '400px', margin: '0 auto' }}>
      ...
      {errors.general && <p style={{ color: 'red', marginBottom: '15px' }}>{errors.general}</p>}
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        <div>
          ...
          <input
            ...
            style={{ width: '100%', padding: '10px', borderColor: errors.username ? 'red' : '#ccc' }}
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            style={{ width: '100%', padding: '10px', borderColor: errors.password ? 'red' : '#ccc' }}
          />
          {errors.password && <span style={{ color: 'red', fontSize: '12px' }}>{errors.password}</span>}
        </div>
        <button type="submit" className="btn" style={{ background: '#d87d4a', color: '#fff', border: 'none', padding: '10px', cursor: 'pointer' }}>
          Login
        </button>
      </form>
    </div>
